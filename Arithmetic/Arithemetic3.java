...
class Arithemetic {
  ...
  public static void main(String [] args) throws IOException{
    ...
    while(true){
      ...
      String f = calculate(t,n,propop); // f for formula
   ...
   * @return the formula, it will be empty if impossible
     */
  private static String calculate(int t,ArrayList<Integer> n, boolean r){
    String formula = "";
   ...
   */
  private static String makeString(String c, ArrayList<Integer> n){
    String formula = "";
    formula += n.get(0);
    for (int j = 0; j < c.length(); j++) {
      formula += c.substring(j,j+1).equals("0") ? (" * " + n.get(j+1) ) : " + " + n.get(j+1);
    }
    return formula;
  }
