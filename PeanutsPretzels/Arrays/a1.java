private static boolean check(int p1, int p2){
  ...
  private static boolean turn(int p1, int p2, boolean yourTurn){
    if (p1 <= 0 && p2 <= 0) return !yourTurn;
    boolean g = !yourTurn;
    //System.out.println(yourTurn + " turn " + p1 + " " + p2);
    for (int i = 0; i < pean.size(); i ++){
      //System.out.println(yourTurn + " turn " + pean.get(i) + " " + pret.get(i));
      if (p1 - pean.get(i) == 0 && p2 - pret.get(i) == 0) {
        if (p1 == peanuts && p2 == pretzels) {
          return yourTurn;
        }
      }else if (p1 - pean.get(i) >= 0 && p2 - pret.get(i) >= 0){
        boolean t = turn(p1 - pean.get(i), p2 - pret.get(i) , !yourTurn);
        if (yourTurn){
          if (!t) g = false;
        }else {
          if (t) g = true;
